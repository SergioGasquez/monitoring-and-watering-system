...
/* This demo uses Timer1 to generate the time base for gathering run time
...
/* In this port, the time base for the run time stats is generated by a 32 bit timer (we have no 64 bit timer available in CC3200).
 ...
 * In this implementation, the value of the 32 bit timer is divided by 1024 and casted to 32 bits in order
 * to compute the statistics. Thus the counter speed is CPU_CLOCK/1024, which is still much faster than  configTICK_RATE_HZ
 * The upper 10 bits available after dividing by 1024 are filled by means of a counting variable that is increased in the timer interrupt
 * timer interrupt is fired every 2^32 clock cycles, so it barely affects performance.
 *
